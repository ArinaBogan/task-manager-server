...
async function getAllTasksDB(id, title) {
...
async function patchTaskDB(id, clientObj) {
  ...
  const sql2 = `update tasks set task=$1,user_id=$2 where id=$3
    returning*`;
  const result = (await client.query(sql2, [newObj.task, newObj.user_id, id]))
    .rows;
  return result;
}
module.exports = { getAllTasksDB, createTaskDB, deleteTaskDB, patchTaskDB };
